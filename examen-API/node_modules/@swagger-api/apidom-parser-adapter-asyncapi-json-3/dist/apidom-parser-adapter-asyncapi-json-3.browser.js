(function (global, factory) {
  ...
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.apidomParserAdapterAsyncapiJson3 = {}, global.fs, global.path));
   ...
   */
  var isNotNull = complement(isNull$1);
   ...
   * RA.isArray({}); //=> false
   */
  var isArray$1 = curryN(1, isFunction(Array.isArray) ? Array.isArray : pipe(type, identical('Array')));
   ...
   * RA.isEmptyArray('42'); // => false
   */
  var isEmptyArray = both(isArray$1, isEmpty);
   ...
   * RA.isNonEmptyArray('42'); // => false
   */
  var isNonEmptyArray = both(isArray$1, isNotEmpty);
   ...
   * RA.isString(1); //=> false
   */
  var isString$1 = curryN(1, pipe(type, identical('String')));
   ...
   * RA.isNonEmptyString(42); // => false
   */
  var isNonEmptyString = allPass([isString$1, isNotObj, isNotEmpty]);
  ...
  var isObject$2 = pipe(type, identical('Object'));
  ...
  var isPlainObj = curryN(1, function (val) {
    if (!isObjLike(val) || !isObject$2(val)) {
      return false;
    }
   ...
   * @public
   */
  let ParseResult$1 = class ParseResult extends minimExports.ArrayElement {
    constructor(content, meta, attributes) {
      super(content, meta, attributes);
  ...
  const isParseResultElement = createPredicate(({
    ...
    return element => element instanceof ParseResult$1 || hasBasicElementProps(element) && isElementType('parseResult', element) && primitiveEq('array', element);
  });
  ...
  class Namespace extends minimExports.Namespace {
    constructor() {
      ...
      this.register('parseResult', ParseResult$1);
    }
  }
   ...
   * @public
   */
  const namespace$1 = new Namespace();
   ...
   * @public
   */
  const isNumber = node => isNodeType('number', node);
  ...
  class JsonProperty extends JsonNode {
    ...
    get value() {
      return this.children.find(node => isFalse(node) || isTrue(node) || isNull(node) || isNumber(node) || isString(node) || isArray(node) || isObject$1(node));
    }
  }
  ...
  class JsonArray extends JsonNode {
    static type = 'array';
    get items() {
      return this.children.filter(node => isFalse(node) || isTrue(node) || isNull(node) || isNumber(node) || isString(node) || isArray(node) || isObject$1);
    }
  }
   ...
   * @public
   */
  let Error$1 = class Error extends Node {
    static type = 'error';
    value;
    isUnexpected;
    constructor({
   ...
   * @public
   */
  const getNodeType$5 = node => node === null || node === void 0 ? void 0 : node.type;

  /**
   * @public
   */
  const isNode$3 = node => typeof getNodeType$5(node) === 'string';
  ...
  const dereference = (object, root) => {
    ...
    return mapObjIndexed(val => {
      if (isPlainObj(val) && has('$ref', val) && propSatisfies(isString$1, '$ref', val)) {
  ...
  // isNode :: Node -> Boolean
  const isNode$2 = pipe(getNodeType$4, isString$1);
  ...
  ParseResult$1.refract = createRefractor$4(ParseResult$1);

  const nodeTypeGetter = node => {
    if (typeof (node === null || node === void 0 ? void 0 : node.type) === 'string') {
      return node.type;
    }
    return getNodeType$4(node);
  };
  const keyMapDefault = {
  ...
  const treeSitterWasm = (typeof document === 'undefined' && typeof location === 'undefined' ? require('u' + 'rl').pathToFileURL(__dirname + '/0327c07adfa215ef7ce8dfd6d8cbfc6b.wasm').href : new URL('0327c07adfa215ef7ce8dfd6d8cbfc6b.wasm', typeof document === 'undefined' ? location.href : document.currentScript && document.currentScript.tagName.toUpperCase() === 'SCRIPT' && document.currentScript.src || document.baseURI).href);
  ...
  if (isFunction(realFetch)) {
    globalThis.fetch = (...args) => {
      // @ts-ignore
      if (isString$1(args[0]) && args[0].endsWith('tree-sitter.wasm')) {
        // @ts-ignore
  	...
  	(function (module, exports$1) {
  		 ...
  		 */ var isFileURI = filename => filename.startsWith("file://");
  		     ...
  		     */ function getValue(ptr, type = "i8") {
  		        ...
  		        /** @param {boolean=} replace */ var relocateExports = (exports$1, memoryBase, replace) => {
  		          var relocated = {};
  		      ...
  		      */ var loadWebAssemblyModule = (binary, flags, libName, localScope, handle) => {
  		       ...
  		       */ function loadDynamicLibrary(libName, flags = {
  		          ...
  		          function moduleLoaded(exports$1) {
  		            if (dso.global) {
  		              mergeLibSymbols(exports$1);
  		            }
  		            dso.exports = exports$1;
  		          }
  		          if (flags.loadAsync) {
  		            return getExports().then(exports$1 => {
  ...
  const treeSitterJson = (typeof document === 'undefined' && typeof location === 'undefined' ? require('u' + 'rl').pathToFileURL(__dirname + '/f951efd18f4932becb741545caa37c8d.wasm').href : new URL('f951efd18f4932becb741545caa37c8d.wasm', typeof document === 'undefined' ? location.href : document.currentScript && document.currentScript.tagName.toUpperCase() === 'SCRIPT' && document.currentScript.src || document.baseURI).href);
  ...
  /* eslint-disable no-underscore-dangle */
  let CstVisitor$1 = class CstVisitor {
    sourceMap = false;
    annotations;
    ParseResultElement = {
      leave: element => {
    ...
    document(node) {
      const element = new ParseResult$1();
      // @ts-ignore
    ...
    object(node) {
      const element = new minimExports.ObjectElement();
      // @ts-ignore
      element._content = node.children;
      this.maybeAddSourceMap(node, element);
      return element;
    ...
    array(node) {
      const element = new minimExports.ArrayElement();
      // @ts-ignore
      element._content = node.children;
      this.maybeAddSourceMap(node, element);
      return element;
    ...
    pair(node) {
      const element = new minimExports.MemberElement();
      // @ts-ignore
    ...
    string(node) {
      const element = new minimExports.StringElement(JSON.parse(node.text));
      this.maybeAddSourceMap(node, element);
      return element;
    }
    number(node) {
      const element = new minimExports.NumberElement(Number(node.text));
      this.maybeAddSourceMap(node, element);
      return element;
    }
    ...
    null(node) {
      const element = new minimExports.NullElement();
      this.maybeAddSourceMap(node, element);
      return element;
    }
    ...
    // eslint-disable-next-line @typescript-eslint/naming-convention
    true(node) {
      const element = new minimExports.BooleanElement(true);
      this.maybeAddSourceMap(node, element);
      return element;
    }
    ...
    // eslint-disable-next-line @typescript-eslint/naming-convention
    false(node) {
      const element = new minimExports.BooleanElement(false);
      this.maybeAddSourceMap(node, element);
      return element;
    }
    ERROR(node, key, parent, path) {
      ...
      if (path.length === 0) {
        ...
        const parseResultElement = new ParseResult$1();
  ...
  const getNodeType$3 = node => {
    ...
    return getNodeType$5(node);
  };

  // @ts-ignore
  const isNode$1 = element => isElement(element) || isNode$3(element);
  ...
  class CstVisitor {
    ...
    ERROR(node, key, parent, path) {
      const errorNode = new Error$1(assignSourceMap({
        children: node.children,
  ...
  const keyMap$4 = {
    ...
    // @ts-ignore
    [Error$1.type]: ['children'],
    ...keyMapDefault$1
  };
  const getNodeType$2 = node => {
    if (isParseResultElement(node)) {
      return 'ParseResultElement';
    }
    if (isElement(node)) {
  ...
  const isNode = element => isElement(element) || isNode$3(element);
  ...
  class JsonAstVisitor {
    ...
    key(node) {
      const element = new minimExports.StringElement(node.value);
      this.maybeAddSourceMap(node, element);
      return element;
    }
    array(node) {
    ...
    number(node) {
      const element = new minimExports.NumberElement(Number(node.value));
      this.maybeAddSourceMap(node, element);
      return element;
    }
  ...
  const analyze = (cst, {
    ...
    const jsonAst = visit$2(rootNode, cstVisitor, {
      // @ts-ignore
      keyMap: keyMap$5,
      state: {
        sourceMap
    ...
    return visit$2(jsonAst.rootNode, astVisitor, {
      // @ts-ignore
      keyMap: keyMap$4,
   ...
   * @public
   */
  const mediaTypes$1 = new AsyncAPIMediaTypes$1('application/vnd.aai.asyncapi;version=3.0.0', 'application/vnd.aai.asyncapi+json;version=3.0.0', 'application/vnd.aai.asyncapi+yaml;version=3.0.0');
   ...
   * @public
   */
  class OperationTraits extends minimExports.ArrayElement {
  ...
  let TraitsVisitor$1 = class TraitsVisitor extends Mixin(SpecificationVisitor, FallbackVisitor) {
    constructor(options) {
      super(options);
      this.element = new OperationTraits();
    }
    ArrayElement(arrayElement) {
      arrayElement.forEach(item => {
        let element;
   ...
   * @public
   */
  class MessageExamples extends minimExports.ArrayElement {
  ...
  class ExamplesVisitor extends Mixin(SpecificationVisitor, FallbackVisitor) {
    constructor(options) {
      super(options);
      this.element = new MessageExamples();
    }
    ArrayElement(arrayElement) {
      arrayElement.forEach(item => {
   ...
   * @public
   */
  class MessageTraits extends minimExports.ArrayElement {
  ...
  class TraitsVisitor extends Mixin(SpecificationVisitor, FallbackVisitor) {
    constructor(options) {
      super(options);
      this.element = new MessageTraits();
    }
    ArrayElement(arrayElement) {
      arrayElement.forEach(item => {
        let element;
   ...
   * @public
   */
  class OperationTraitSecurity extends minimExports.ArrayElement {
  ...
  class SecurityVisitor extends Mixin(SpecificationVisitor, FallbackVisitor) {
    constructor(options) {
      super(options);
      this.element = new OperationTraitSecurity();
    }
    ArrayElement(arrayElement) {
      arrayElement.forEach(item => {
        const securityRequirementElement = this.toRefractedElement(['document', 'objects', 'SecurityRequirement'], item);
  ...
  const jsonMediaTypes = new AsyncAPIMediaTypes$1(
    ...mediaTypes$1.filterByFormat("generic"),
    ...mediaTypes$1.filterByFormat("json")
  );
  ...
  const detectionRegExp = new RegExp('"asyncapi"\\s*:\\s*"(?<version_json>3\\.(?:[1-9]\\d*|0)\\.(?:[1-9]\\d*|0))"');
  const detect = (source) => __async(null, null, function* () {
    return detectionRegExp.test(source) && (yield detect$1(source));
  });
  const parse = (_0, ..._1) => __async(null, [_0, ..._1], function* (source, options = {}) {
    const refractorOpts = propOr({}, "refractorOpts", options);
    const parserOpts = omit(["refractorOpts"], options);
    const parseResultElement = yield parse$1(source, parserOpts);
    const { result } = parseResultElement;
    if (isNotUndefined(result)) {
      const asyncApiElement = AsyncApi3.refract(result, refractorOpts);
      asyncApiElement.classes.push("result");
      parseResultElement.replaceResult(asyncApiElement);
    }
    return parseResultElement;
  });
  const namespace = createNamespace(asyncApi3);

  exports.detect = detect;
  exports.detectionRegExp = detectionRegExp;
  exports.mediaTypes = jsonMediaTypes;
