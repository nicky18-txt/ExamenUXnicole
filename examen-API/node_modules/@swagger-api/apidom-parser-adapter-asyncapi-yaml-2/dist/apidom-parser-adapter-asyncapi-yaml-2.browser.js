(function (global, factory) {
  ...
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.apidomParserAdapterAsyncapiYaml2 = {}, global.fs, global.path));
  ...
  /* eslint-disable class-methods-use-this */
  class GenericMapping extends Tag$1 {
  ...
  /* eslint-disable class-methods-use-this */
  class GenericSequence extends Tag$1 {
  ...
  class GenericString extends Tag$1 {
   ...
   */
  const formatFlowPlain = pipe(normalizeLineBreaks, trim, collapseLineBreakToSpace, split('\n'), map(trimStart$1), join('\n'));
  ...
  const formatFlowSingleQuoted = pipe(normalizeLineBreaks, trim, collapseLineBreakToSpace, split('\n'), map(trimStart$1), join('\n'), removeQuotes("'"));

  /**
   * Formats Flow Scalar Double-Quoted style.
   * https://yaml.org/spec/1.2/spec.html#id2787109
   */
  const formatFlowDoubleQuoted = pipe(normalizeLineBreaks, trim, preventLineBreakCollapseToSpace, collapseLineBreakToSpace, distExports.unraw, split('\n'), map(trimStart$1), join('\n'), removeQuotes('"'));
  ...
  /* eslint-disable class-methods-use-this */
  let Boolean$1 = class Boolean extends Tag$1 {
  ...
  /* eslint-disable class-methods-use-this */
  class FloatingPoint extends Tag$1 {
  ...
  class JsonSchema extends FailsafeSchema {
    constructor() {
       ...
       */
      this.registerTag(new Boolean$1(), true);
   ...
   * @public
   */
  const isNode$3 = node => typeof getNodeType$3(node) === 'string';
  ...
  // isNode :: Node -> Boolean
  const isNode$2 = pipe(getNodeType$2, isString);
  ...
  const isNode$1 = node => Array.isArray(node) || isNode$3(node);
  ...
  const isNode = node => isElement(node) || isNode$3(node) || Array.isArray(node);
  ...
  class YamlAstVisitor {
    ...
    comment(node) {
      ...
      if (isStreamComment) {
        // @ts-ignore
        const element = new Comment(node.content);
  ...
  const analyze = (cst, {
    ...
    const yamlAst = visit$2(rootNode, cstVisitor, {
      // @ts-ignore
      keyMap: keyMap$4,
    ...
    return visit$2(yamlAst.rootNode, astVisitor, {
      // @ts-ignore
      keyMap: keyMap$3,
  ...
  const detect$1 = async source => {
    try {
      const cst = await analyze$1(source);
  ...
  const parse$1 = async (source, {
    ...
    const cst = await analyze$1(source);
    const syntacticAnalysisResult = analyze(cst, {
      sourceMap
    });
    cst.delete();
  ...
  const detectionRegExp = new RegExp(`(?<YAML>^(["']?)asyncapi\\2\\s*:\\s*(["']?)(?<version_yaml>2\\.(?:[1-9]\\d*|0)\\.(?:[1-9]\\d*|0))\\3(?:\\s+|$))|(?<JSON>"asyncapi"\\s*:\\s*"(?<version_json>2\\.(?:[1-9]\\d*|0)\\.(?:[1-9]\\d*|0))")`, "m");
  ...
  exports.mediaTypes = yamlMediaTypes;
